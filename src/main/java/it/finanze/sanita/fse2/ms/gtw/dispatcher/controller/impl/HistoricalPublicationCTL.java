...
public class HistoricalPublicationCTL extends AbstractCTL implements IHistoricalDocPublicationCTL {
    ...
    @Override
    public PublicationCreationResDTO historicalDocPublicationCreation(PublicationCreationReqDTO requestBody, MultipartFile file, HttpServletRequest request) {
        ...
        if(!validationCFG.getSaveValidationErrorOnly() || (validationCFG.getSaveValidationErrorOnly() && out!=null)){
            try {
                ...
                kafkaSRV.notifyPublicationEvent(jsonObj, result, true, false);
            } catch (Exception e) {
	...
	private PublicationOutputDTO handleCDA(PublicationCreationReqDTO jsonObj, byte[] bytePDF) {
        ...
        if (!StringUtility.isNullOrEmpty(cda)){
            ...
            final boolean matchingHash = cdaFacadeSRV.validateHash(hashedCDA, jsonObj.getTransactionID());
            if (!matchingHash) {
